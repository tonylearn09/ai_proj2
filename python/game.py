...
class Gomoku:
    ...
    def isEnd(self, debug = False): 
        ...
        def boardState(pos): 
            ...
            if (pos[0] < 0 or pos[0] >= 17 or pos[1] < 0 or pos[1] >= len(self.gomokuboard[pos[0]])):
                ...
                return self.gomokuboard[pos[0]][pos[1]]
    ...
    def print_board(self):
        for index,row in enumerate(self.gomokuboard):
            if(index == 0 or index == 16 ):
                print('        ', end='')
                for e in row:
                    print(e, end=' ')
                print('')
            elif(index == 1 or index == 15):
                print('       ', end='')
                for e in row:
                    print(e, end=' ')
                print('')
            elif(index == 2 or index == 14):
                print('      ', end='')
                for e in row:
                    print(e, end=' ')
                print('')
...
def updateFeature(gomoku, pos_index):
    ...
    if nextMove_i <= 8:
        ...
        startPos['rdiag'] = [(nextMove_i-delta, nextMove_j) for delta in range(0, 5) \
                                if nextMove_i-delta>= 0 and nextMove_j<len(gomokuboard[nextMove_i-delta])]
        #print(startPos['rdiag'])
        ...
        for delta in range(0, 5):
            if nextMove_i + delta >= 8:
                ...
                #if nextMove_j >= 0:
                    #valid_pos.append(nextMove_i+delta, nextMove_j-delta)
                valid_pos.add((nextMove_i+delta, nextMove_j))
       ...
       # print(valid_pos)
        ...
        valid_pos.update([(nextMove_i+delta, nextMove_j) for delta in range(0, 5) \
                          if nextMove_i+delta < 17 and nextMove_j<len(gomokuboard[nextMove_i+delta])])
        #print(valid_pos)
        ...
        valid_pos.update([(nextMove_i+delta, nextMove_j-delta) for delta in range(0, 5) \
                          if (nextMove_i+delta < 17 and nextMove_j-delta >= 0)])
